...
use crate::{NavNode, SpawnPoint, SpawnTeam, Vec2, Vec3, WallSegment, Material, MapData, compute_bbox2d};
...
fn edges_equal(e1: &(Vec2, Vec2), e2: &(Vec2, Vec2)) -> bool {
    ...
    (e1.0.x - e2.0.x).abs() < TOL && (e1.0.y - e2.0.y).abs() < TOL &&
    (e1.1.x - e2.1.x).abs() < TOL && (e1.1.y - e2.1.y).abs() < TOL
}
...
pub fn load_map_from_nav(path: &std::path::Path) -> Result<MapData, String> {
    ...
    let nav: NavMesh = serde_json::from_reader(reader)
        .map_err(|e| format!("Failed to parse nav JSON: {}", e))?;
    ...
    for (_area_id, area) in &nav.areas {
        // Get all corners for this area (3D)
        let corners: Vec<_> = area.corners.iter()
            .map(|c| Vec3::from(*c))
            .collect();
        
        // Get 2D corners (XY plane)
        let corners_2d: Vec<_> = corners.iter()
        ...
        if area.connections.is_empty() {
            ...
            spawn_points.push(SpawnPoint {
                position: centroid,
                team: if area.dynamic_attribute_flags & 0x1 != 0 { SpawnTeam::T } else { SpawnTeam::CT },
                angle: 90.0,
            });
        }
    }
    ...
    let walls: Vec<WallSegment> = wall_edges.iter()
    ...
    let nav_nodes: Vec<NavNode> = nav.areas.iter()
        .map(|(id_str, area)| {
            let corners_3d: Vec<_> = area.corners.iter()
                .map(|c| Vec3::from(*c))
                .collect();
            ...
            let corners_2d: Vec<Vec2> = corners_3d.iter()
                .map(|c| Vec2::new(c.x, c.y))
                .collect();
...
pub fn simplify_walls(mut walls: Vec<WallSegment>) -> Vec<WallSegment> {
    ...
    walls.sort_by(|a, b| {
        let ord = a.start.x.partial_cmp(&b.start.x).unwrap_or(std::cmp::Ordering::Equal);
        if ord != std::cmp::Ordering::Equal { ord }
        else { a.start.y.partial_cmp(&b.start.y).unwrap_or(std::cmp::Ordering::Equal) }
    });
    walls.dedup_by(|a, b| {
    ...
    walls.sort_by(|a, b| {
        ...
        if ord != std::cmp::Ordering::Equal { ord }
            ...
            let ord2 = a.start.y.partial_cmp(&b.start.y).unwrap_or(std::cmp::Ordering::Equal);
            if ord2 != std::cmp::Ordering::Equal { ord2 }
            else { 
                let ord3 = a.end.x.partial_cmp(&b.end.x).unwrap_or(std::cmp::Ordering::Equal);
                if ord3 != std::cmp::Ordering::Equal { ord3 }
                else { a.end.y.partial_cmp(&b.end.y).unwrap_or(std::cmp::Ordering::Equal) }
            }
        }
    });
        ...
        for existing in &mut merged {
            ...
            if are_collinear(&existing.start, &existing.end, &wall.start, tolerance) ||
               are_collinear(&existing.start, &existing.end, &wall.end, tolerance) {
                ...
                let direction = Vec2::new(existing.end.x - existing.start.x, existing.end.y - existing.start.y);
                ...
                if len > tolerance {
                    ...
                    let to_start = Vec2::new(wall.start.x - line_start.x, wall.start.y - line_start.y);
                    ...
                    if perp_start < tolerance && perp_end < tolerance {
                        ...
                        let combined_start = Vec2::new(
                            existing.start.x.min(wall.start.x).min(existing.end.x).min(wall.end.x),
                            existing.start.y.min(wall.start.y).min(existing.end.y).min(wall.end.y)
                        );
                        let combined_end = Vec2::new(
                            existing.start.x.max(wall.start.x).max(existing.end.x).max(wall.end.x),
                            existing.start.y.max(wall.start.y).max(existing.end.y).max(wall.end.y)
                        );
...
pub fn load_map_by_name(map_name: &str) -> Option<MapData> {
    ...
    for prefix in &tri_extensions {
        ...
        for search_path in &tri_search_paths {
            ...
            if tri_path.exists() {
                if let Ok(map_data) = MapData::load_from_tri(&tri_path) {
...
mod tri_tests {
    ...
    fn create_test_tri_file() -> NamedTempFile {
        ...
        for val in &triangle {
            file.write_all(&val.to_le_bytes()).expect("Failed to write triangle");
        }
    ...
    fn test_load_map_from_tri_basic() {
        ...
        assert!(result.is_ok(), "Failed to load tri file: {:?}", result.err());
        
        let map = result.unwrap();
        assert!(!map.name.is_empty());
        assert!(map.walls.len() >= 3, "Expected at least 3 walls from triangle, got {}", map.walls.len());
    }
    ...
    fn test_load_map_from_tri_multiple_triangles() {
        ...
        let map = result.unwrap();
        assert!(map.walls.len() >= 3, "Expected at least 3 walls from triangles, got {}", map.walls.len());
    }
    ...
    fn test_empty_tri_file() {
        ...
        assert!(result.is_ok(), "Failed to load empty tri file: {:?}", result.err());
        let map = result.unwrap();
    ...
    fn test_flat_triangle_no_walls() {
        ...
        assert!(map.walls.is_empty(), "Flat triangle should produce no walls (height < threshold)");
    }
    ...
    fn test_bvh_raycast() {
        ...
        assert!(map.line_blocked_3d(origin, direction), "Ray should be blocked by wall BVH");
    }
}
