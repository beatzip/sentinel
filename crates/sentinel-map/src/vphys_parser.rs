...
use crate::kv3_parser::{KV3Value, bytes_to_f32_vec, bytes_to_i32_vec, parse_kv3};
...
impl VPhysData {
    ...
    pub fn from_kv3(kv: &KV3Value) -> Result<Self, String> {
        ...
        let version = obj.get("version").and_then(|v| v.as_i64()).unwrap_or(3) as u32;

        let mut parts = Vec::new();
        if let Some(parts_arr) = obj.get("m_parts")
            && let Some(parts_array) = parts_arr.as_array()
        {
    ...
    pub fn load_vphys_as_mapdata(path: &std::path::Path) -> Result<crate::data::MapData, String> {
        ...
        let walls: Vec<crate::data::WallSegment> = triangles
            .iter()
            .filter_map(|triangle| {
                ...
                Some(vec![
                    crate::data::WallSegment {
                        start: v1,
                        end: v2,
                        material: crate::data::Material::Solid,
                    },
                    crate::data::WallSegment {
                        start: v2,
                        end: v3,
...
impl VPhysShape {
    fn from_kv3(kv: &KV3Value) -> Option<Self> {
        ...
        if let Some(meshes_val) = obj.get("m_meshes")
            && let Some(meshes_arr) = meshes_val.as_array()
        {
        ...
        if let Some(hulls_val) = obj.get("m_hulls")
            && let Some(hulls_arr) = hulls_val.as_array()
        {
...
impl MeshData {
    fn from_kv3(kv: &KV3Value) -> Option<Self> {
        ...
        if let Some(attrs_val) = obj.get("m_CollisionAttributes")
            && let Some(attrs_arr) = attrs_val.as_array()
        {
            for attr_val in attrs_arr {
                if let Some(attr_obj) = attr_val.as_object()
                    && let Some(group_str) = attr_obj.get("m_CollisionGroupString")
                    && let Some(group) = group_str.as_str()
                {
...
impl HullData {
    fn from_kv3(kv: &KV3Value) -> Option<Self> {
        ...
        if let Some(hull_obj) = obj.get("m_Hull")
            && let Some(hull_data) = hull_obj.as_object()
        {
            ...
            if let Some(faces_val) = hull_data.get("m_Faces")
                && let Some(faces_arr) = faces_val.as_array()
            {
                for face_val in faces_arr {
                    if let Some(face_bytes) = face_val.as_bytes()
                        && let Ok(indices) = bytes_to_i32_vec(face_bytes)
                    {
                        faces.push(indices);
                    }
                }
            ...
            if let Some(edges_val) = hull_data.get("m_Edges")
                && let Some(edges_arr) = edges_val.as_array()
            {
                for edge_val in edges_arr {
                    if let Some(edge_bytes) = edge_val.as_bytes()
                        && let Ok(indices) = bytes_to_i32_vec(edge_bytes)
                    {
                        edges.push(indices);
                    }
                }
