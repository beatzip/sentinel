...
- [ ] Gate 1A.7 Runtime ResourceSystem capture: define and collect an auditable capture that proves the same serialized `CStrongHandle` token resolves to a canonical `CModel` resource for the declared build; a raw pointer dump or unrelated process/build is unsupported
