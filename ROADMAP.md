...
- [x] Gate 1A historical-session boundary: completed contract clarification; an ended demo process is `historical_session_unrecoverable` by default, while any reviewed cross-session determinism profile is research-only and cannot promote historical handles without separately human-approved reproduction and coverage evidence
