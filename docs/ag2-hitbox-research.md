...
The same local-only trace now records raw `m_iItemDefinitionIndex` with each sampled pawn rather than a lookup-derived display name. A zero definition index remains only an observed zero, not a default-model conclusion. A sampled non-zero definition index is a valid *schema lookup key*, but not model identity: it must still be resolved by a hash-recorded `scripts/items/items_game.txt` from a VPK proven build-matched to the exact demo, then by deterministic asset extraction and declared dependency resolution.

The supplied `build_agent_model_manifest.py` is appropriate for that narrow schema stage. It parses a supplied `items_game.txt`, retains only `model_player` paths under `characters/models/`, and writes the source SHA-256 with `def_index → model_player` entries. It neither validates demo build matching nor reads VPK resources, resolves dependencies, verifies a VMDL hash, or cross-links a pawn/tick; it must therefore not itself be treated as a verified-model manifest. No local `items_game.txt`, VPK directory, or build provenance was supplied in this workspace, so the script was inspected but not executed.
