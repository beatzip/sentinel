...
### Gate 1A.6 definition-index correlation — `historical_mapping_required`

The selected demo observes controller field `CCSPlayerController.m_nPawnCharacterDefIndex` as `uint16` value `5308` at tick 2. The current/19-Aug catalog contains exactly one `model_player` path for current definition index `5308`: `agents/models/ctm_fbi/ctm_fbi_variantb.vmdl`, with prefab `customplayertradable`; no second current path shares that definition index. This establishes a current catalog definition-to-path record only.
